 ...
 * AR scene with cones placed in real world at fixed positions.
 *
 * Source: https://threejs.org/examples/webxr_ar_cones.html
 *
 * This example demonstrates:
 * - AR session setup with ARButton
 * - Simple AR placement without hit-test (objects at origin)
 * - local-floor reference space for proper positioning
 * - Transparent background for camera passthrough
 *
 * Note: AR requires a compatible device (phone with ARCore/ARKit).
 * Without hit-test, objects appear at fixed positions relative to user.
 */

export const ARConesSimple = () => {
    ...
    // Setup AR button and session
    useEffect(() => {
        const renderer = $$(rendererRef)
        if (!renderer || !$$(arSupported)) return

        // Create AR button with local-floor for proper positioning
    ...
    useFrame((state) => {
        ...
        const scene = $$(sceneRef)
        if (scene) {
            // Animate cones with gentle rotation
            scene.children.forEach((child, i) => {
                if (child instanceof THREE.Mesh && child.name.startsWith('cone')) {
                    child.rotation.y = time * 0.5 + i * 0.3
    ...
    // Define cone positions in a circle around user
    ...
    for (let i = 0; i < 8; i++) {
        ...
        conePositions.push({
            ...
            color: new Color().setHSL(i / 8, 0.7, 0.5).getHex()
        })
    }
    ...
    return (
        <canvas3D>
            ...
            <scene ref={sceneRef} background={null as unknown as THREE.Color}>
                ...
                <mesh name="center-marker" position={[0, 0.1, 0]}>
