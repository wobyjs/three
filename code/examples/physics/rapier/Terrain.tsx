...
/**
 * Port of physics_rapier_terrain from Three.js examples.
 * Demonstrates terrain collision with physics objects.
 *
 * Source: https://threejs.org/examples/physics_rapier_terrain.html
 *
 * Key features:
 * - Heightfield terrain
 * - Rolling physics objects
 * - Dynamic collision response
 *
 * Note: Simplified version without actual Rapier integration.
 * For full physics, use RAPIER.Heightfield with RAPIER.ColliderDesc.
 */
...
export const Terrain = () => {
    ...
    useEffect(() => {
        // Generate terrain height data with hills and valleys
        for (let i = 0; i <= TERRAIN_SEGMENTS; i++) {
            ...
            for (let j = 0; j <= TERRAIN_SEGMENTS; j++) {
                ...
                heightData[i][j] =
                    ...
                    Math.cos(x * 0.2) * Math.sin(z * 0.2) * 1
            }
        }
    ...
    useFrame((state) => {
        ...
        balls.forEach(ball => {
            ...
            if (ball.mesh.position.y < terrainH + ball.radius) {
                ...
                // Roll along terrain slope
                const rollForce = new Vector3(-normal.x, 0, -normal.z).multiplyScalar(8)
    ...
    return (
        <canvas3D>
            ...
            <perspectiveCamera fov={60} aspect={window.innerWidth / window.innerHeight} near={0.1} far={200} position={[25, 20, 25]} />
